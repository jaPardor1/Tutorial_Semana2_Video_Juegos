...
def system_collision_player_enemy(world:esper.World,player_entity:int,level_cfg:dict):
    components = world.getComponents(CSurface,CTransform,CTagEnemy)
    pl_t = world.component_for_entity(player_entity,CTransform,CTagEnemy)
    pl_s = world.component_for_entity(player_entity,CSurface)
    pl_rect = pl_s.surf.get_rect(topleft=pl_t.pos)
    
    for enemy_entity,(c_s,c_t,_) in components:
        ene_rect = c_s.surf.get_rect(topleft=pl_t.pos)
