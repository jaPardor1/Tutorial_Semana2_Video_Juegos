...
def system_collision_player_enemy(world:esper.World,player_entity:int,level_cfg:dict,expl_info:dict):
    components = world.get_components(CSurface,CTransform,CTagEnemy)
    pl_t = world.component_for_entity(player_entity,CTransform)
    pl_s = world.component_for_entity(player_entity,CSurface)
    pl_rect = pl_s.area.copy()#pl_s.surf.get_rect(topleft = pl_t.pos)
    ...
    for enemy_entity,(c_s, c_t ,_) in components:
        ...
        if ene_rect.colliderect(pl_rect):
            ...
            create_explosion(world,c_t.pos,expl_info)
